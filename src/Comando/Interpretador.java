...
import Comando.ComandoRead;
import Variavel.*;
import Comando.*;
import Comando.Comando;
import Comando.ComandoAtrib;
import Comando.ComandoElse;
import Comando.ComandoEndp;
import Comando.ComandoIf;
import Comando.ComandoRead;
import Comando.ComandoWriteStr;
import Comando.ComandoWriteVar;
import Comando.ComandoWriteln;
...
public class Interpretador {
    ...
    public void leArquivo() {
        ...
        do {
            ...
            if (comandoAtual.equals("endp")) {
                ...
                linha++;
            } else if (comandoAtual.equals("writeErr")) {
                comandoAtual = arq.proximaPalavra();
                comandoAtual = arq.proximaPalavra();
                trataComandoWriteErr(linha, comandoAtual);
                linha++;
            } else if (comandoAtual.equals("writeColor")) {
                usoucolor = true;
                comandoAtual = arq.proximaPalavra();//[ ou (
                if (comandoAtual.equals("[")) {
                    color = arq.proximaPalavra();//cor selecionada
                    comandoAtual = arq.proximaPalavra();//]
                    comandoAtual = arq.proximaPalavra();
                } else {
                    color = "default";
                }
                comandoAtual = arq.proximaPalavra();//palavra
                trataComandoWriteColor(linha, comandoAtual, color);
                linha++;
            } else if (comandoAtual.equals("setColor")) {
                comandoAtual = arq.proximaPalavra();
                comandoAtual = arq.proximaPalavra();//cor selecionada
                trataComandoSetColor(linha, comandoAtual);
                linha++;
    ...
    private void trataComandoWriteErr(int lin, String txt) {

        ComandoWriteErr c = new ComandoWriteErr(lin, txt);
        comandos.addElement(c);
    }

    private void trataComandoWriteColor(int lin, String txt, String color) {

        ComandoWriteColor c = new ComandoWriteColor(lin, txt, color);
        comandos.addElement(c);
    }

    private void trataComandoSetColor(int lin, String color) {

        ComandoSetColor c = new ComandoSetColor(lin, color);
        comandos.addElement(c);
    }
